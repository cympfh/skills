...
1. 対象リポジトリを決める
   - カレントが git repo ならそれを使う
   - ユーザーが `owner/repo` や URL を出していればそれ
   - **`cympfh/*` 以外は触らない。** それ以外なら止めて報告する
...
| 最小実装して PR を出す | `cympfh/*` 以外の repo を触る |
...
- `/pickup auto` — 確認なしで1件選んで進める（それでも `cympfh/*`・最小実装・1件制限は守る）
